...
export class Program {
    ...
    constructor(
        ...
        this._logTracker = logTracker ?? new LogTracker(undefined, 'FG');
        ...
        return this._runEvaluatorWithCancellationToken(token, () => {
            ...
            if (openFiles.length > 0) {
                ...
                for (const sourceFileInfo of openFiles) {
                    if (targetOnly && !sourceFileInfo.uri.getFilePath().startsWith(targetOnly)) {
                        continue;
                    }
    ...
    getCacheUsage() {
        ...
        const entryCountRatio = typeCacheEntryCount / 7500000;
        const fileCountRatio = this._parsedFileCount / 10000;
    ...
    private _isImportAllowed(importer: SourceFileInfo, importResult: ImportResult, isImportStubFile: boolean): boolean {
        ...
        if (!this._configOptions.useLibraryCodeForTypes) {
            return importResult.importType === ImportType.Local && !importer.isThirdPartyImport;
        }
